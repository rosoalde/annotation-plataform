...
export default function AnnotateView() {
    ...
    const confirmField = async (recId: string, key: string) => {
        ...
        // Se guarda YA: si no se persiste aquí, un cambio de opinión más
        // tarde (Deshacer → CONFIRMO otra vez) no queda reflejado hasta
        // pulsar "Guardar y siguiente", y una recarga muestra la decisión
        // anterior en vez de esta.
        try {
            await annotationsApi.saveField(projectId!, {
                record_id: recId, project_id: projectId!, field_name: key,
                original_text: llmVal, corrected_text: llmVal,
                is_correction: false, correction_reason: llmJustif,
            });
            setRefreshRecordId(recId);
            qc.invalidateQueries({ queryKey: ["records", projectId, "annotate"] });
        } catch (err) {
            console.error(err);
            showToast("Error al guardar la confirmación", "warn");
        }
    };
    const confirmTopic = async (recId: string) => {
        ...
        // Topic y Sentimiento comparten fila: se reenvía el sentimiento ya
        // guardado por este anotador (o el del LLM si aún no decidió) para
        // no pisarlo.
        const currentSentiment = rec?.annotator_sentiment ?? rec?.sentiment_llm ?? 2;
        try {
            await annotationsApi.saveSentiment(projectId!, {
                record_id: recId, project_id: projectId!,
                original_sentiment: rec?.sentiment_llm ?? 2, corrected_sentiment: currentSentiment,
                is_correction: false,
                original_topic: rec?.topic_llm ?? undefined, corrected_topic: rec?.topic_llm ?? "",
                topic_reason: rec?.justif_topic ?? "",
            });
            setRefreshRecordId(recId);
            qc.invalidateQueries({ queryKey: ["records", projectId, "annotate"] });
        } catch (err) {
    ...
    const confirmSentiment = async (recId: string) => {
        ...
        // Mismo cuidado que en confirmTopic: no pisar el topic ya guardado.
        const currentTopic = rec?.annotator_topic ?? rec?.topic_llm ?? "";
        try {
            await annotationsApi.saveSentiment(projectId!, {
                record_id: recId, project_id: projectId!,
                original_sentiment: rec?.sentiment_llm ?? 2, corrected_sentiment: rec?.sentiment_llm ?? 2,
                is_correction: false, correction_reason: rec?.justif_sentimiento ?? "",
                original_topic: rec?.topic_llm ?? undefined, corrected_topic: currentTopic,
            });
            setRefreshRecordId(recId);
            qc.invalidateQueries({ queryKey: ["records", projectId, "annotate"] });
        } catch (err) {
    ...
    const confirmPilar = async (recId: string, key: string) => {
        setConfirmedFields((prev) => ({ ...prev, [recId]: new Set([...(prev[recId] ?? []), key]) }));
        setRejecting((prev) => { const s = new Set(prev[recId] ?? []); s.delete(key); return { ...prev, [recId]: s }; });
        const rec = (data?.records ?? []).find((r) => r.id === recId);
        ...
        try {
            await annotationsApi.savePilar(projectId!, {
                record_id: recId, project_id: projectId!, pilar: key,
                original_value: llmVal ?? 2, corrected_value: llmVal ?? 2,
                is_correction: false, correction_reason: llmJustif,
            });
            setRefreshRecordId(recId);
            qc.invalidateQueries({ queryKey: ["records", projectId, "annotate"] });
        ...
        mutationFn: async ({ rec, ann }: { rec: AnnotRecord; ann: RecordAnn }) => {
            ...
            const calls: Promise<unknown>[] = [
                annotationsApi.saveSentiment(projectId!, {
                    ...
                    is_correction: confirmed.has("sentiment") ? false : rejected.has("sentiment") ? true : false,
            ...
            for (const p of PILARS) {
                ...
                calls.push(annotationsApi.savePilar(projectId!, {
                    ...
                    is_correction: confirmed.has(p.key) ? false : rejected.has(p.key) ? true : false,
                    correction_reason: pa.reason ?? undefined,
                }));
            }
            ...
            for (const f of TEXT_FIELDS) {
                ...
                calls.push(annotationsApi.saveField(projectId!, {
                    ...
                    is_correction: confirmed.has(f.key) ? false : rejected.has(f.key) ? true : false, correction_reason: fa.reason ?? undefined,
                }));
            }

            await Promise.all(calls);
            // Única acción que marca el registro como terminado para este
            // anotador (ver done_ids en el backend) — los guardados por
                                                ...
                                                <div key={f.key} style={S.smallCard}>
                                                    ...
                                                    {isConfirmed ? (
                                                        <>
                                                            ...
                                                            {(rec as any)[`annotator_${f.key}`] && (rec.annotator_corrections ?? []).includes(f.key) && (
                                                ...
                                                <div key={p.key} style={S.smallCard}>
                                                    ...
                                                    {isConfirmed ? (
                                                        <>
                                                            ...
                                                            {/* ✓ CONFIRMADO ANOTADOR (ROJO si es corrección) */}
                                                            {(rec as any)[`annotator_${p.key}`] !== undefined && (rec.annotator_corrections ?? []).includes(p.key) && (
                                                                <div style={{ ...S.confirmedTag, borderLeft: "3px solid #e05252", background: "rgba(224,82,82,0.12)" }}>
                                                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                                                                        <div style={{ flex: 1 }}>
                                                                            <div style={{ fontSize: 10, fontWeight: 600, color: "#e05252" }}>
